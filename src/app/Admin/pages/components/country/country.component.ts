...
///////////////////////////////////////////div1
  ...
  divOnePopup(locationData: any): void {
    // Clear the current popup container
    this.popupContainer.clear();
  
    // Create a component factory for Division1Component
    const factory = this.componentFactoryResolver.resolveComponentFactory(Division1Component);
  
    // Dynamically create the component
    const componentRef = this.popupContainer.createComponent(factory);
  
    // Pass data (if needed) to the created component
    componentRef.instance.locationData = locationData;
  
    // Add a closePopup function for cleanup
    componentRef.instance.closePopup = () => {
      this.popupContainer.clear();
    };
  
    // Log to verify data flow
    console.log('Popup created with location data:', locationData);
    this.states;
  }
